...
export const Route = createFileRoute("/_marketing/shop")({
  ...
  loader: async ({ context: { queryClient }, deps: { page } }) => {
    await queryClient.ensureQueryData(
      convexQuery((api as any).commerce.products.listPublished, {
        page,
        perPage: 12,
      }),
    );
...
function ShopContent() {
  ...
  const { data: categories } = useSuspenseQuery(
    convexQuery((api as any).commerce.categories.listPublic, {}) as any,
  ) as {
    data: Array<{
      _id: string;
      name: string;
  ...
  const filteredProducts = data.products.filter((product) => {
    ...
    const matchesSearch =
      ...
      product.excerpt?.toLowerCase().includes(searchParam.toLowerCase());
