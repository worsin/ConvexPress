...
**Positioning:** internal extension (`commerceDigital`).
**Extension gate:** ``commerce.digital.digitalProductsEnabled`` in the Settings system; `requireX(ctx)` helper on every mutation/query. Admin UI hides the nav item when disabled.
**Code lives at:** `ConvexPress-Admin/packages/backend/convex/commerceDigital/`

**Consumes these ConvexPress systems:**

- **Product System** — digital products are a product sub-type.
- **Media System** — downloadable assets via signed URLs.
- **Order System** — on paid order, generates time/IP-limited download tokens.
- **Email Notification System** — delivers download links post-purchase.

**WooCommerce analog:** WooCommerce Downloadable Products + Easy Digital Downloads — signed-URL delivery with license key support.
