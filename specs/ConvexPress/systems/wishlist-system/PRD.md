...
**Positioning:** internal extension (`commerceWishlists`).
**Extension gate:** ``commerce.wishlists.wishlistsEnabled`` in the Settings system; `requireX(ctx)` helper on every mutation/query. Admin UI hides the nav item when disabled.
**Code lives at:** `ConvexPress-Admin/packages/backend/convex/commerceWishlists/`

**Consumes these ConvexPress systems:**

- **Users** — authenticated wishlists are keyed by `users._id`; guest wishlists live in localStorage until signup-merge.
- **Product System** — wishlist entries reference `commerce_products`.
- **Email Notification System** — optional back-in-stock / price-drop alerts.

**WooCommerce analog:** TI WooCommerce Wishlist / YITH WooCommerce Wishlist — saved-for-later lists with sharing.
