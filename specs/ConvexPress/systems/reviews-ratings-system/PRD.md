...
**Positioning:** internal extension (`commerceReviews`).
**Extension gate:** ``commerce.reviews.reviewsEnabled`` in the Settings system; `requireX(ctx)` helper on every mutation/query. Admin UI hides the nav item when disabled.
**Code lives at:** `ConvexPress-Admin/packages/backend/convex/commerceReviews/`

**Consumes these ConvexPress systems:**

- **Product System** — reviews attach to `commerce_products._id`.
- **Comment System** — ConvexPress reuses comment infrastructure (moderation, spam, threading) for product reviews.
- **Users** — reviewers are either logged-in `users` or `verified_purchaser` guests.
- **Order System** — verified-purchase flag requires matching order.
- **SEO System** — AggregateRating structured data per product page.

**WooCommerce analog:** WooCommerce reviews (comment-based) + Customer Reviews for WooCommerce.
