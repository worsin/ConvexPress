...
**Code lives at:** `ConvexPress-Admin/packages/backend/convex/commerce/checkout.ts`

**Consumes these ConvexPress systems:**

- **Cart System** — converts an active cart into a checkout session.
- **Tax System** — authoritative tax calc via `calculateTaxForLinesFromRules`, gated by `commerce.payments.subscriptionChargingEnabled` for subscriptions.
- **Shipping System** — rate selection + address validation.
- **Payment System** — creates Stripe PaymentIntent / SetupIntent; writes `commerce_payment_transactions`.
- **Order System** — on successful payment, materializes `commerce_orders` + `commerce_order_items`.
- **Event Dispatcher** — emits `checkout.started`, `checkout.shipping_set`, `checkout.completed`.

**WooCommerce analog:** WooCommerce Checkout block + `WC_Checkout` class; three-step (shipping → payment → review) flow.
