...
- **Payload:**
  ...
  ```
- **Subscribers:**
  - Email: "Password Reset Request" (`[redacted-airtable-record-id]`) -- **conditional** on `sendPasswordResetEmail` setting (default: false)
...
- **Payload:**
  ...
  ```
- **Subscribers:**
  - Email: "Password Changed Confirmation" (`[redacted-airtable-record-id]`) -- **conditional** on `sendPasswordChangedEmail` setting (default: true)
  - Site Notification: "Password Changed" success toast (`[redacted-airtable-record-id]`)
...
- **Airtable Route ID:** `[redacted-airtable-record-id]`
...
- **Shared With:** User Profile System (`[redacted-airtable-record-id]`)
...
|------|-------------|-------|------------|----------|---------|-------------|
| Password Reset Request | `[redacted-airtable-record-id]` | `password.reset_requested` | Customer (requesting user) | Immediate | "Reset your password for {site_name}" | Only if `sendPasswordResetEmail` = true (default: false) |
| Password Changed Confirmation | `[redacted-airtable-record-id]` | `password.changed` | Customer (user whose password changed) | Immediate | "Your password was changed" | Only if `sendPasswordChangedEmail` = true (default: true) |
...
|------|-------------|-------|------|-----------|------------|---------|
| Password Changed | `[redacted-airtable-record-id]` | `password.changed` | Success (toast) | No | Customer (user) | "Your password was changed successfully" |
...
|--------|------|---------------|
| **Auth System** (`[redacted-airtable-record-id]`) | **Hard** | Convex Auth integration, shared webhook handler in `convex/http.ts`, session management, `requireRole()` helper, `getUserIdentity()`. Without Auth, nothing works. |
...
| **User Profile System** (`[redacted-airtable-record-id]`) | **Medium** | Shares the `/dashboard/settings` route. Password section is embedded within the Profile System's settings page. |
...
| Priority | P1 - High |
| Complexity | Simple |
| Depends On | Auth System (`[redacted-airtable-record-id]`) |

### Routes

| Record ID | Name | Path | App | Auth |
|-----------|------|------|-----|------|
| `[redacted-airtable-record-id]` | Forgot Password | `/forgot-password` | Website | No |
| `[redacted-airtable-record-id]` | Reset Password | `/reset-password` | Website | No |
| `[redacted-airtable-record-id]` | Account Settings | `/dashboard/settings` | Website | Yes |

### Actions

| Record ID | Name | Action Code |
|-----------|------|-------------|
| `[redacted-airtable-record-id]` | Request Password Reset | `password.request_reset` |
| `[redacted-airtable-record-id]` | Reset Password | `password.reset` |
| `[redacted-airtable-record-id]` | Change Password | `password.change` |

### Events

| Record ID | Name | Event Code |
|-----------|------|------------|
| `[redacted-airtable-record-id]` | Password Reset Requested | `password.reset_requested` |
| `[redacted-airtable-record-id]` | Password Changed | `password.changed` |
| `[redacted-airtable-record-id]` | Password Reset Completed | `password.reset_completed` |

### Email Notifications

| Record ID | Name | Triggered By |
|-----------|------|-------------|
| `[redacted-airtable-record-id]` | Password Reset Request | `password.reset_requested` |
| `[redacted-airtable-record-id]` | Password Changed Confirmation | `password.changed` |
