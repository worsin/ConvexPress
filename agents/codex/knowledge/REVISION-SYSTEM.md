...
|---|------|-------|------------|----------|---------|------------|-----------------|
| 1 | Revision Restored Alert | `revision.restored` | Post author (Employee) | Immediate | "Post revision restored by {user}" | Only when restorer != post author | `[redacted-airtable-record-id]` |
...
|---|------|-------|---------|------|------------|-----------|-----------------|
| 1 | Revision Created | `revision.created` | `Revision #{number} saved for "{title}"` | Info | No (transient toast) | Author who saved | `[redacted-airtable-record-id]` |
| 2 | Revision Restored | `revision.restored` | `Revision restored for "{title}"` | Warning | Yes (stays in notification center) | Post author | `[redacted-airtable-record-id]` |
...
|--------|--------|------|--------|
| **Post System** | `[redacted-airtable-record-id]` | **Hard** | Revisions are snapshots of posts. `parentId` references `posts` table. Post System calls `internal.revisions.createOnSave` on every save and `internal.revisions.deleteByParent` on permanent delete. |
| **Role & Capability System** | `[redacted-airtable-record-id]` | **Hard** | Permission checks (`revision.view`, `revision.compare`, `revision.restore`, `revision.delete`) use the capability system. |
| **Auth System** | `[redacted-airtable-record-id]` | **Hard** | All queries/mutations require Convex Auth authentication. `authorId` is a user identifier. |
| **Settings System** | `[redacted-airtable-record-id]` | **Soft** | Reads `max_revisions` and `revisions_enabled` settings. Falls back to defaults (25 and true) if settings not configured. |

### Depended On By

| System | Record | Type | Reason |
|--------|--------|------|--------|
| **Post System** | `[redacted-airtable-record-id]` | **Soft** | Post System calls `internal.revisions.createOnSave` before applying updates. Edit screen shows Revisions metabox. |
