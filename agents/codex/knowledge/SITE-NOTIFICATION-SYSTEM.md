...
- **Events:** `notification.site_sent` (`[redacted-airtable-record-id]`)
- **Errors:**
...
- **Email Notification System** (`[redacted-airtable-record-id]`): Both consume events from the Event Dispatcher independently. Site notifications write to Convex; email notifications send via Resend. User preferences control each channel independently per notification type.
