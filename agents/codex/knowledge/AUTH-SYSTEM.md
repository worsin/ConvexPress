...
**Airtable Expert ID:** `[redacted-airtable-record-id]`
**Status:** Complete (100%)
...
1. Check Airtable Tech Stack Updates first:
   - Base: `[redacted-airtable-base-id]`
   - Table: `[redacted-airtable-table-id]`
...
- **Payload:**
  ...
  ```
- **Subscribers:**
  - Email: "Login from New Device" (`[redacted-airtable-record-id]`) -- Subject: "New login detected from {device}"
  - Site: "Login from New Location" (`[redacted-airtable-record-id]`) -- Warning, Persistent
...
- **Payload:**
  ...
  ```
- **Subscribers:**
  - Email: "Failed Login Attempts" (`[redacted-airtable-record-id]`) -- Admin notification, immediate
  - Site: "Failed Login Alert" (`[redacted-airtable-record-id]`) -- Error, Persistent, to the affected user
...
|------|-------------|-------|------------|----------|------------------|
| Login from New Device | `[redacted-airtable-record-id]` | `auth.logged_in` | Customer (the user) | Immediate | `New login detected from {device}` |
| Failed Login Attempts | `[redacted-airtable-record-id]` | `auth.login_failed` | Admin (all administrators) | Immediate | `Multiple failed login attempts detected` |

### Site Notifications

| Name | Airtable ID | Event | Type | Persistent | Recipients |
|------|-------------|-------|------|-----------|------------|
| Login from New Location | `[redacted-airtable-record-id]` | `auth.logged_in` | Warning | Yes | Customer (the user) |
| Failed Login Alert | `[redacted-airtable-record-id]` | `auth.login_failed` | Error | Yes | Customer (affected user) |
...
|--------|------|---------------|
| **Registration System** (`[redacted-airtable-record-id]`) | **Hard** | User creation via webhook, identity resolution, auth session for invitation flow |
| **Role & Capability System** (`[redacted-airtable-record-id]`) | **Hard** | `getCurrentUser(ctx)` for all permission checks, `ctx.auth.getUserIdentity()` for identity |
| **User Profile System** (`[redacted-airtable-record-id]`) | **Hard** | User record created by auth webhook, session for profile editing |
| **Password Management System** (`[redacted-airtable-record-id]`) | **Hard** | Convex Auth handles all password operations, webhook for password change detection |
| **Post System** (`[redacted-airtable-record-id]`) | **Hard** | `requireAuth()` / `requireRoleOrHigher()` for all post mutations |
| **Page System** (`[redacted-airtable-record-id]`) | **Hard** | Same auth helpers for page mutations |
| **Media System** (`[redacted-airtable-record-id]`) | **Hard** | Auth helpers for media operations |
| **Comment System** (`[redacted-airtable-record-id]`) | **Hard** | Auth helpers for comment operations |
| **Event Dispatcher System** (`[redacted-airtable-record-id]`) | **Hard** | Identity context for event emission |
...
- [x] **Login error handling** - Login page detects Convex Auth error redirect params (`?error=...&error_description=...`), displays user-friendly error message, and records failed attempt via `[redacted-airtable-record-id]` mutation.
...
4. On failure, return structured error for `[redacted-airtable-record-id]` integration
...
| Field | Value |
|-------|-------|
| Record ID | `[redacted-airtable-record-id]` |
| Name | Auth System |
| Category | User & Auth |
| Priority | P0 - Critical |
| Complexity | Medium |
...
|-----------|------|------|-----|------|
| `[redacted-airtable-record-id]` | Login | `/login` | Website | No |

### Actions

| Record ID | Name | Action Code |
|-----------|------|-------------|
| `[redacted-airtable-record-id]` | Login | `auth.login` |
| `[redacted-airtable-record-id]` | Logout | `auth.logout` |
| `[redacted-airtable-record-id]` | OAuth Login | `auth.oauth_login` |
| `[redacted-airtable-record-id]` | Refresh Session | `auth.refresh_session` |
| `[redacted-airtable-record-id]` | Verify Email | `auth.verify_email` |

### Events

| Record ID | Name | Event Code |
|-----------|------|------------|
| `[redacted-airtable-record-id]` | User Logged In | `auth.logged_in` |
| `[redacted-airtable-record-id]` | User Logged Out | `auth.logged_out` |
| `[redacted-airtable-record-id]` | OAuth Login | `auth.oauth_completed` |
| `[redacted-airtable-record-id]` | Email Verified | `auth.email_verified` |
| `[redacted-airtable-record-id]` | Login Failed | `auth.login_failed` |

### Email Notifications

| Record ID | Name | Triggered By |
|-----------|------|-------------|
| `[redacted-airtable-record-id]` | Login from New Device | `auth.logged_in` |
| `[redacted-airtable-record-id]` | Failed Login Attempts | `auth.login_failed` |

### Site Notifications

| Record ID | Name | Triggered By |
|-----------|------|-------------|
| `[redacted-airtable-record-id]` | Login from New Location | `auth.logged_in` |
| `[redacted-airtable-record-id]` | Failed Login Alert | `auth.login_failed` |
