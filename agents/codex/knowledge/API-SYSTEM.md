...
- **Payload:**
  ...
  ```
- **Subscribers:**
  - Site Notification: `[redacted-airtable-record-id]` -- Info: "New API key created: {name}" (Admin, persistent)
...
- **Payload:**
  ...
  ```
- **Subscribers:**
  - Email Notification: `[redacted-airtable-record-id]` -- "Webhook delivery failed: {endpoint}" (Admin, Immediate, only when `success === false`)
  - Site Notification: `[redacted-airtable-record-id]` -- Error: "Webhook to {endpoint} failed" (Admin, persistent, only when `success === false`)
...
|------|-------|------------|----------|---------|-----------|
| Webhook Failure Alert (`[redacted-airtable-record-id]`) | `api.webhook_triggered` | Admin | Immediate | Webhook delivery failed: {webhook_name} | Only when `success === false` |
...
|------|-------|------|-----------|------------|---------|
| API Key Created (`[redacted-airtable-record-id]`) | `api.key_created` | Info | Yes | Admin | New API key created: {name} |
| Webhook Failed (`[redacted-airtable-record-id]`) | `api.webhook_triggered` | Error | Yes | Admin | Webhook to {endpoint} failed |
...
|--------|-----------|---------------|----------------|
| **Auth System** | `[redacted-airtable-record-id]` | **Hard** | auth identity for admin management operations. API key `userId` references user identifier. Cannot function without auth for admin operations. |
| **Role & Capability System** | `[redacted-airtable-record-id]` | **Hard** | `manage_options` / `manage_api_keys` capability check for all management actions. API key scopes map to WordPress capabilities. Cannot create/manage keys without capability checks. |
