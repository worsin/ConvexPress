...
|--------|---------------|-----------------|
| **Post System** (`[redacted-airtable-record-id]`) | **Hard** | Post counts (by status), recent published posts, draft posts for Activity and At a Glance widgets. Quick Draft creates posts via the Post System's `posts` table. Dashboard cannot function without the Post System. |
| **Comment System** (`[redacted-airtable-record-id]`) | **Hard** | Comment counts (by status), recent comments for Activity widget, pending moderation count for Moderation Queue widget. Dashboard is significantly degraded without comments. |
