...
- Recommended next action: Monitor `convex` minor releases; on each upgrade run `bun scripts/check-type-honesty.mjs` with a relaxed allowlist to see if the cascade has shrunk, then trim suppressions and allowlist accordingly.
