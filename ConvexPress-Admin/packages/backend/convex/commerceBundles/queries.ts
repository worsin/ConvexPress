 ...
 * Delegates to canonical runtime helpers for consistent pricing.
 */
export const calculatePrice = query({
  ...
  handler: async (ctx: any, args: any) => {
    ...
    const snapshot = args.selections
      ? await resolveBundleSelectionSnapshot(ctx, { bundle, components, selections: args.selections })
      : await resolveBundlePricingPreview(ctx, { bundle, components });

    if (!snapshot) return null;

    return {
      regularPrice: snapshot.regularPriceAmount,
      bundlePrice: snapshot.resolvedBundlePriceAmount,
      savings: snapshot.regularPriceAmount - snapshot.resolvedBundlePriceAmount,
      savingsPercent: snapshot.regularPriceAmount > 0
        ? Math.round(((snapshot.regularPriceAmount - snapshot.resolvedBundlePriceAmount) / snapshot.regularPriceAmount) * 100)
        : 0,
    };
  },
});
 ...
 * Delegates to canonical runtime helpers for consistent stock checking.
 */
export const checkAvailability = query({
  ...
  handler: async (ctx: any, args: any) => {
    ...
    if (!bundle || bundle.status !== "active") {
      return { available: false, reason: "Bundle not active" };
    }
    ...
    if (!snapshot) return { available: false, reason: "Invalid configuration" };

    return resolveBundleAvailability(ctx, {
      bundle,
      snapshot,
