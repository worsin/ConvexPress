  ...
  handler: async (ctx, args) => {
    ...
    if (!user) {
      return {
        ...
        counts: { all: 0, publish: 0, draft: 0, pending: 0, private: 0, trash: 0, future: 0 },
      };
    }
    ...
    const counts = {
      ...
      draft: allPagesUnfiltered.filter((p) => p.status === "draft" || p.status === "auto-draft").length,
    ...
    const pagesWithAuthors = await Promise.all(
      paginatedPages.map(async (pg) => {
        const author = pg.authorId ? await ctx.db.get("users", pg.authorId) : null;
        return {
          ...pg,
          author: author
            ? {
                ...
                displayName:
                  (author as { displayName?: string }).displayName ?? author.email,
                email: author.email,
              }
            : null,
        };
...
export const counts = query({
  ...
  handler: async (ctx) => {
    ...
    return {
      ...
      draft: allPages.filter((p) => p.status === "draft" || p.status === "auto-draft").length,
...
export const getFrontPage = query({
  ...
  handler: async (ctx) => {
    ...
    try {
      const frontPage = await ctx.db.get("posts", staticHomepageId as Id<"posts">);

      if (!frontPage || frontPage.type !== "page" || frontPage.status !== "publish") {
        return null;
      }
