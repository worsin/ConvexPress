...
async function isBridgeEnabled(ctx: any): Promise<boolean> {
	...
	try {
		...
		// Settings read failure should not block the bridge.
	}

	return true;
}

async function isLiveSubscriptionChargingEnabled(ctx: any): Promise<boolean> {
	const settingsRow = await ctx.db
		.query("settings")
		.withIndex("by_section", (q: any) =>
			q.eq("section", "commerce.payments"),
		)
		.unique();
	const values = (settingsRow?.values ?? {}) as Record<string, unknown>;
	return values.subscriptionChargingEnabled === true;
}
	...
	handler: async (ctx, args) => {
		...
		if (intent.status === "activated" && intent.subscriptionId) {
			...
			return {
				...
				trialDays: intent.pricingSnapshot?.trialDays ?? 0,
				currentPeriodEndAt:
		...
		if (
			args.paymentResult.provider === "free" &&
			(initialAmount > 0 || recurringAmount > 0)
		) {
			throw new ConvexError({
				code: "VALIDATION_ERROR",
				message:
					"Only zero-amount free checkout intents can be activated with the free payment provider.",
			});
		}
		...
		const status: "trialing" | "active" = trialDays > 0 ? "trialing" : "active";
		...
		const subscriptionId = await ctx.db.insert("commerce_subscriptions", {
			...
			defaultPaymentMethodId:
				args.paymentResult.paymentMethodId ?? intent.savedPaymentMethodId,
			stripeCustomerId:
		...
		// Create entitlements from offer.entitlementCodes and bridge them into
		// membership immediately when the bridge is enabled.
		const codes = Array.from(
			new Set((offer.entitlementCodes ?? []).filter(Boolean)),
		);
		for (const code of codes) {
			await ctx.db.insert("commerce_subscription_entitlements", {
				...
				status:
					status === "active" || status === "trialing" ? "active" : "grace",
				startsAt: now,
				endsAt: undefined,
