...
function asCanonicalJsonValue(value: unknown, path: string): JsonValue {
  if (value === null) {
    return null;
  }
  ...
  if (Array.isArray(value)) {
    return value.map((item, index) =>
      asCanonicalJsonValue(item, `${path}[${index}]`),
    );
  }
  ...
  if (typeof value === "object" && value !== null) {
    ...
    for (const key of Object.keys(source).sort()) {
      ...
      result[key] = asCanonicalJsonValue(item, `${path}.${key}`);
    }
    return result;
  }
...
export function canonicalJson(value: unknown): string {
  return JSON.stringify(asCanonicalJsonValue(value, "$"));
}
...
/** A small synchronous SHA-256 implementation so the root contract stays browser-safe. */
