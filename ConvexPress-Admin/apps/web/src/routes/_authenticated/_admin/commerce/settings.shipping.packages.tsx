...
function ShippingPackagesPage() {
  // PRD A3 — wired to v2 backend.
  const packages = useQuery(
    (api as any).shipping.packages.queries.list,
    { includeArchived: false },
  ) as any[] | undefined;

  const createPkg = useMutation(
    (api as any).shipping.packages.mutations.create,
  );
  const updatePkg = useMutation(
    (api as any).shipping.packages.mutations.update,
  );
  const deletePkg = useMutation(
    (api as any).shipping.packages.mutations.remove,
  );
    ...
    try {
      ...
      await updatePkg({
        ...
        patch: {
          ...
          tareWeight: editWeight ? parseFloat(editWeight) : undefined,
          dimensions,
        },
      });
