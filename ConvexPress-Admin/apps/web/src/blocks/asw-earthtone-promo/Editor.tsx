...
import type { AswEarthtonePromoAttrs } from "./schema";

const emptyDetail: AswEarthtonePromoAttrs["details"][number] = {
  label: "",
  value: "",
};

export function AswEarthtonePromoEditor({
  attrs,
  onChange,
  disabled,
}: BlockEditorProps<AswEarthtonePromoAttrs>) {
  const updateDetail = (index: number, patch: Partial<AswEarthtonePromoAttrs["details"][number]>) => {
    onChange({
      ...attrs,
