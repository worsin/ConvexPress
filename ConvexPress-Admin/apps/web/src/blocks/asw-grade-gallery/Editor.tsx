...
import type { AswGradeGalleryAttrs, AswGradeImage } from "./schema";

const emptySection: AswGradeGalleryAttrs["sections"][number] = {
  grade: "Grade",
  description: "",
  notes: "",
  images: [],
...
const emptyImage: AswGradeImage = {
  mediaId: "",
  alt: "",
  caption: "",
};

export function AswGradeGalleryEditor({
  attrs,
  onChange,
  disabled,
}: BlockEditorProps<AswGradeGalleryAttrs>) {
  const updateSection = (
    index: number,
    patch: Partial<AswGradeGalleryAttrs["sections"][number]>,
  ) => {
    onChange({
      ...attrs,
  ...
  const updateImage = (sectionIndex: number, imageIndex: number, patch: Partial<AswGradeImage>) => {
          ...
          >
            ...
            <div className="grid gap-3 border border-border bg-background p-3">
              ...
              {section.images.map((image, imageIndex) => (
                ...
                >
                  <MediaField label="Image" value={image.mediaId} disabled={disabled} promptSeed={`${section.grade} tonewood`} onChange={(mediaId) => updateImage(sectionIndex, imageIndex, { mediaId })} />
