...
import type { ContactStackAttrs } from "./schema";

const emptyItem: ContactStackAttrs["items"][number] = {
  label: "",
  value: "",
  href: "",
};

export function ContactStackEditor({
  attrs,
  onChange,
  disabled,
}: BlockEditorProps<ContactStackAttrs>) {
  const updateItem = (index: number, patch: Partial<ContactStackAttrs["items"][number]>) => {
    onChange({
      ...attrs,
