...
import type { ProductCollectionItem, ProductCollectionAttrs } from "./schema";

const emptyProduct: ProductCollectionItem = {
  title: "",
  summary: "",
  href: "",
  price: "",
...
const emptyGroup: ProductCollectionAttrs["groups"][number] = {
  label: "Product group",
  productIds: [],
  products: [],
};
...
export function ProductCollectionEditor({
  attrs,
  onChange,
  disabled,
}: BlockEditorProps<ProductCollectionAttrs>) {
  const updateProduct = (index: number, patch: Partial<ProductCollectionItem>) => {
    onChange({
      ...attrs,
  ...
  const updateGroup = (
    index: number,
    patch: Partial<ProductCollectionAttrs["groups"][number]>,
  ) => {
    onChange({
      ...attrs,
