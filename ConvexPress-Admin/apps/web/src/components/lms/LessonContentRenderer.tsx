  ...
  if (mediaId) {
    return (
      <figure className="space-y-2">
        <AdminMediaImage mediaId={mediaId as Id<"media">} alt={alt} />
...
function AdminMediaImage({ mediaId, alt }: { mediaId: Id<"media">; alt: string }) {
  const media = useQuery(api.media.queries.get, { mediaId }) as
    | { url?: string | null; altText?: string | null; title?: string | null }
    | null
    | undefined;

  if (media === undefined) {
    return <div className="h-56 w-full animate-pulse bg-muted" aria-hidden="true" />;
  }
  if (!media?.url) return null;
  return (
    <img
      src={media.url}
      alt={alt || media.altText || media.title || ""}
      loading="lazy"
