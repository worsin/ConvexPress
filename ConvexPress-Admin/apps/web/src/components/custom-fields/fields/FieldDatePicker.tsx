...
export function FieldDatePicker({ field, value, onChange, labelPlacement, instructionPlacement }: FieldRendererProps) {
  const settings = useMemo(() => { try { return JSON.parse(field.settings); } catch { return {}; } }, [field.settings]);
  const displayFormat = settings.displayFormat ?? "yyyy-MM-dd";
  const firstDay = settings.firstDay ?? 1;
