...
4. The expert knowledge doc (from `.claude/docs/{SYSTEM-NAME}.md`)
...
|---|--------|---------------|---------------|
| 1 | API System Expert | `/experts:api-system` | `.claude/docs/API-SYSTEM.md` |
| 2 | Audit Log System Expert | `/experts:audit-log-system` | `.claude/docs/AUDIT-LOG-SYSTEM.md` |
| 3 | Auth System Expert | `/experts:auth-system` | `.claude/docs/AUTH-SYSTEM.md` |
| 4 | Comment System Expert | `/experts:comment-system` | `.claude/docs/COMMENT-SYSTEM.md` |
| 5 | Content Editor System Expert | `/experts:content-editor-system` | `.claude/docs/CONTENT-EDITOR-SYSTEM.md` |
| 6 | Custom Field System Expert | `/experts:custom-field-system` | `.claude/docs/CUSTOM-FIELD-SYSTEM.md` |
| 7 | Dashboard System Expert | `/experts:dashboard-system` | `.claude/docs/DASHBOARD-SYSTEM.md` |
| 8 | Email Notification System Expert | `/experts:email-notification-system` | `.claude/docs/EMAIL-NOTIFICATION-SYSTEM.md` |
| 9 | Event Dispatcher System Expert | `/experts:event-dispatcher-system` | `.claude/docs/EVENT-DISPATCHER-SYSTEM.md` |
| 10 | Media System Expert | `/experts:media-system` | `.claude/docs/MEDIA-SYSTEM.md` |
| 11 | Menu System Expert | `/experts:menu-system` | `.claude/docs/MENU-SYSTEM.md` |
| 12 | Page System Expert | `/experts:page-system` | `.claude/docs/PAGE-SYSTEM.md` |
| 13 | Password Management System Expert | `/experts:password-management-system` | `.claude/docs/PASSWORD-MANAGEMENT-SYSTEM.md` |
| 14 | Post System Expert | `/experts:post-system` | `.claude/docs/POST-SYSTEM.md` |
| 15 | Registration System Expert | `/experts:registration-system` | `.claude/docs/REGISTRATION-SYSTEM.md` |
| 16 | Revision System Expert | `/experts:revision-system` | `.claude/docs/REVISION-SYSTEM.md` |
| 17 | Role & Capability System Expert | `/experts:role-capability-system` | `.claude/docs/ROLE-CAPABILITY-SYSTEM.md` |
| 18 | Routing System Expert | `/experts:routing-system` | `.claude/docs/ROUTING-SYSTEM.md` |
| 19 | RSS/Feed System Expert | `/experts:rss-feed-system` | `.claude/docs/RSS-FEED-SYSTEM.md` |
| 20 | Search System Expert | `/experts:search-system` | `.claude/docs/SEARCH-SYSTEM.md` |
| 21 | SEO System Expert | `/experts:seo-system` | `.claude/docs/SEO-SYSTEM.md` |
| 22 | Settings System Expert | `/experts:settings-system` | `.claude/docs/SETTINGS-SYSTEM.md` |
| 23 | Site Notification System Expert | `/experts:site-notification-system` | `.claude/docs/SITE-NOTIFICATION-SYSTEM.md` |
| 24 | Sitemap System Expert | `/experts:sitemap-system` | `.claude/docs/SITEMAP-SYSTEM.md` |
| 25 | Taxonomy System Expert | `/experts:taxonomy-system` | `.claude/docs/TAXONOMY-SYSTEM.md` |
| 26 | User Profile System Expert | `/experts:user-profile-system` | `.claude/docs/USER-PROFILE-SYSTEM.md` |
| 27 | AI Content Generation Expert | `/experts:ai-content-generation` | `.claude/docs/AI-CONTENT-GENERATION.md` |
| 28 | Analytics System Expert | `/experts:analytics-system` | `.claude/docs/ANALYTICS-SYSTEM.md` |
| 29 | GA4 Integration System Expert | `/experts:ga4-integration-system` | `.claude/docs/GA4-INTEGRATION-SYSTEM.md` |
| 30 | Tabbed Editor Shell Expert | `/experts:tabbed-editor-shell` | `.claude/docs/TABBED-EDITOR-SHELL.md` |

#### Admin UI Experts (4)

| # | Expert | Slash Command | Status | Knowledge Doc |
|---|--------|---------------|--------|---------------|
| 29 | Admin Shell & Navigation UI Expert | `/experts:admin-shell-ui` | Complete | `.claude/docs/ADMIN-SHELL-UI.md` |
| 30 | Admin List Table UI Expert | `/experts:admin-list-table-ui` | Complete | `.claude/docs/ADMIN-LIST-TABLE-UI.md` |
| 31 | Admin Editor Layout UI Expert | `/experts:admin-editor-ui` | Complete | `.claude/docs/ADMIN-EDITOR-UI.md` |
...
The admin app can run as both a **web app** (browser) and an **Electron desktop app** (DMG/EXE/AppImage).

The Electron app uses a **two-tier install flow**:
- **Server mode**: First-time setup -- installs ConvexPress, creates the first administrator account, and provisions the Convex database
- **Client mode**: Connects to an existing ConvexPress instance by entering the site URL

The desktop packaging lives in `ConvexPress-Admin/packages/desktop/` and uses electron-builder for cross-platform distribution.
...
| **Desktop** | Electron | Desktop packaging for admin app |
| **Desktop Config** | electron-store | Persistent local config (install mode, site URL) |
| **Desktop Build** | electron-builder | Cross-platform packaging (DMG/EXE/AppImage) |
| **AI Routing** | OpenRouter | Multi-model AI routing for content generation |
| **Package Manager** | Bun | |
| **Monorepo** | Turborepo | |
...
│   └── packages/desktop/      # Electron main process, preload, IPC, packaging
...
API keys for external services (AI/OpenRouter, Email/Resend, Search) are **configurable from the admin UI** via Settings pages, not just environment variables. The backend uses a **settings-first pattern**: it checks the `settings` table for a stored API key before falling back to `process.env`. This means administrators can configure and rotate keys without redeploying.
