...
**Airtable System ID:** `recSG6RltDScSL69w`
...
|------|-------------|-------|------------|----------|------------------|
| Welcome Email | `recRphjsGI169Dfxr` | `registration.user_registered` | Customer (new user) | Immediate | `Welcome to {site_name}!` |
| Email Verification | `recyWu026qdM7svC0` | `registration.user_registered` | Customer (new user) | Immediate | `Verify your email for {site_name}` |
| User Invitation | `rec3la18fitvRNPvf` | `registration.user_invited` | Customer (invited person) | Immediate | `You've been invited to {site_name}` |
| New User Notification (Admin) | `recLUv2jk8YU0oG4k` | `registration.user_registered` | Admin (all administrators) | Batched | `New user registered: {user_email}` |
...
|------|-------------|-------|------|-----------|------------|
| New User Registered | `recwjUjw0rzKdkZTI` | `registration.user_registered` | Info | Yes (admin feed) | All Administrators |
| User Invited | `recGgfiuc7pBlXAcF` | `registration.user_invited` | Success | No (toast) | Specific admin who invited |
...
|--------|------|---------------|
| **Auth System** (`recNGEVtMvLjp6o8h`) | **Hard** | Convex Auth integration, webhook HTTP endpoint, session management, `requireRole()` helper, Svix verification |
